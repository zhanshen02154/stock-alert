...
async def get_all_sessions(
    req: Request, session_service: SessionService = Depends(get_session_service)
...
async def get_session_messages(
    req: Annotated[MessagesRequest, Query()],
...
async def remove_session(
    chat_id: str, session_service: SessionService = Depends(get_session_service)
) -> ApiResponse[None]:
    try:
...
async def create_session(
    ...
    try:
        ...
        session_info = session_service.create_session(
            session_id=session_id, user_id=str(user_id), title="新对话"
        )
...
async def add_message(
    req: Request,
    params: Annotated[ChatRequest, Form()],
    chat_svc: ChatService = Depends(get_chat_service),
):
    try:
        user_id: int = req.state.user_id
        message_id = chat_svc.add_message(
            message=params.content, session_id=params.chat_id, user_id=user_id
        )
...
async def get_ai_response(
    ...
    async def event_generator():
        try:
            async for message in chat_svc.chat_astream(
                session_id=params.chat_id, user_id=user_id
            ):
